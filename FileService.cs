...
namespace Talent.Common.Services
{
    public class FileService : IFileService
    {
        ...
        public async Task<string> GetFileURL(string id, FileType type)
        {
            ...
            try
            {
                fileURL = await _awsService.GetPresignedUrlObject(id, "mvpstudioadvancetaskbucket");
                return fileURL;
            }
            catch
            {
        ...
        public async Task<string> SaveFile(IFormFile file, FileType type)
        {
            ...
            if (file != null && type == FileType.ProfilePhoto && pathWeb != "")
            {
                    using (var fileStream = new FileStream(path, FileMode.Create))
                    {
                        ...
                        if (!await _awsService.PutFileToS3(path, fileStream, "mvpstudioadvancetaskbucket"))
                        {
                        path = "";
                        }
        ...
        {
            await _awsService.RemoveFileFromS3(id, "mvpstudioadvancetaskbucket");
            return true;
        }
