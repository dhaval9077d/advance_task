    ...
    {   
        ...
        public async Task<IActionResult> GetTalentSnapshots([FromQuery]FeedIncrementModel feed)
        {
            try
            {
                ...
                var result = await _profileService.GetTalentSnapshotList(id, false, 0, 5);
        ...
        public ActionResult AddLanguage([FromBody] AddLanguageViewModel language)
        {
            try
            {
                if (language.Id == null)
                {
                    _profileService.AddNewLanguage(language, _userAppContext.CurrentUserId);
                    return Json(new { Success = true });
                }
                else
                {
        ...
        public async Task<ActionResult> DeleteLanguage([FromBody] AddLanguageViewModel language)
        {
            try
            {
                if (language.Id != null)
                {
                    await _profileService.RemoveLanguage(language);
                    return Json(new { Success = true });
                }
                else
                {
        ...
        public ActionResult AddSkill([FromBody]AddSkillViewModel skill)
        {
            try
            {
                if (skill.Id == null)
                {
                    _profileService.AddNewSkill(skill, _userAppContext.CurrentUserId);
                    return Json(new { Success = true });
                }
                else
                {
        ...
        public async Task<IActionResult> UpdateSkill([FromBody]AddSkillViewModel skill)
        {
            try
            {
                await _profileService.UpdateSkill(skill, _userAppContext.CurrentUserId);
                return Json(new { Success = true });
            }
            catch
            {
        ...
        public async Task<IActionResult> DeleteSkill([FromBody]AddSkillViewModel skill)
        {
            try
            {
                await _profileService.RemoveSkill(skill);
                return Json(new { Success = true });
            }
            catch
            {
        ...
        public ActionResult AddExperience([FromBody]ExperienceViewModel experience)
        {
            try
            {
                ...
                {
                    _profileService.AddNewExperience(experience, _userAppContext.CurrentUserId);
                    return Json(new { Success = true });
                }
                else
                {
        ...
        public async Task<ActionResult> DeleteExperience([FromBody] ExperienceViewModel experience)
        {
            try
            {
                if (experience.Id != null)
                {
                    await _profileService.RemoveExperience(experience);
                    return Json(new { Success = true });
                }
                else
                {
        ...
        public async Task<string> SaveImageAsync(IFormFile imageFile)
        {
            ...
            var imagePath = Path.Combine(_profileImageFolder, "Images", imageName);
